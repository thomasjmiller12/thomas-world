...
async function runVisitorInput(
  ...
  const obs = await buildDelta(agentId);
  const inputText = `${obs.text}\n\n## A visitor speaks to you\n${visitorName || "A visitor"} (here with you) says: "${text}"\n\nReply by simply writing what you say back — it's spoken to them, streamed word-for-word. You keep all your tools (walk somewhere, make something, check your memory). When the conversation has run its course, say your goodbye and call leave_chat in the same message.`;
