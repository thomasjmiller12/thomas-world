...
const live = process.env.OPENAI_LIVE_TEST === "1";

describe.skipIf(!live)("OpenAI provider live smoke", () => {
  it(
    "executes a tool, completes the next model round, and resumes its persisted native thread",
    async () => {
      const marker = "town-provider-resume-42";
      ...
      const first = await openaiProvider.runTurn({
        agentId: "builder",
      ...
      const resumed = await openaiProvider.runTurn({
        agentId: "builder",
        model: { provider: "openai", model: "gpt-5.4" },
        systemPrompt:
