...
> **⚠️ M3 continuity rewrite in progress (branch `thomas/m3-continuity`, NOT yet deployed — live Railway still runs the M2.1 architecture).** Source of truth: vault doc `~/Documents/Personal/Projects/Thomas's Town — Memory & Continuity Architecture.md` — read it before touching the agent loop. The model: each agent is now ONE continuous, self-compacting thread (the Claude Code model) instead of a chatbot re-prompted from scratch every tick. **Phases 1–2 done & committed:** `runtime/loop.ts` (the unified continuous-thread loop, `runTurn` + `runTick`; server-side compaction `compact-2026-01-12` @ 50K trigger; per-agent thread persisted as JSONB in `agent_threads` via `engine/thread.ts`; incremental message-cache breakpoint stripped on persist) replaces the deleted `runtime/tick.ts`; reflection runs as a turn on the thread; idle ticks moved Haiku → **Sonnet 4.6** (compaction needs Sonnet+); `buildObservation` → `buildDelta` (lean push/pull delta: standing state + notice-push, self-events + elsewhere-events excluded). **Phase 3 (next):** speech unification (plain assistant text = speak aloud; deletes `say`/say-boost/separate chat session) + chats-in-thread (visitor message = interrupt-push input) + input queue. **Phase 4:** cutover + delete `chat.ts` session machinery + tune. Migration 0006 is additive (no DB wipe). When M3 deploys, the M2.1 bullets below are superseded.
