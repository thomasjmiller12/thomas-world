...
- **Frontend is still scripted simulation** (`apps/web/src/game/systems/AgentSimulator.ts` + `data/simulation-scripts.ts`) — replacing it with a `WorldClient` over the live API is Milestone 2. **The M2 source of truth is the vault doc `~/Documents/Personal/Projects/Thomas's Town — M2 Interaction Design.md`** (interaction ladder, paced scenes + group chat, world effects, contract v2 deltas, build order A–G) — read it before any M2 work; plan §8 defers to it. Design mocks for M2 UI: `design/town-concepts-handoff/` (note: brief + screenshots live under `project/uploads/` and `project/screenshots/`).
