...
- OpenAI uses SDK `MemorySession` plus Responses inline compaction. Do not reintroduce standalone compaction or slice stored history at arbitrary checkpoint positions: SDK 0.16.1 prunes standalone retained inputs on replay. `history-session.test.ts` exercises the actual next request across inline compaction and reload.
- Current pursuits are a small structured `/.pursuits` metadata row in `memory_files`, separate from identity and the `/.cursor` perception row. Only `update_pursuits` edits this working set; generic memory commands are confined to `/memories/`. Never bulk-clear the table. Core notes carry edit dates; linked artifact/capability state is refreshed from canonical records each turn.
- Reflection can verify evidence with an explicit read-tool allowlist and edit only its own memory/pursuits. Automatic episodic retention/recall is restricted to source-linked public work under the reserved `town_public` tag; model-authored memories cannot use that tag. Filter work before limiting events, and include capability decisions whose target is `payload.agent` rather than the event actor.
- Hindsight connectivity is not ingestion readiness. `/health/memory` reports cached per-bank counts without content. Its PostgreSQL runtime needs `pg_trgm` operators in its search path; the extension belongs in `public` alongside `vector`, even when Hindsight tables use a separate schema. Verify an isolated retain/recall cycle after infrastructure changes.
- Resident creation inventory defaults to made work, with explicit diary/all scopes and pagination. Long artifact/repository/note reads use bounded continuation and version/hash guards. Never suppress a reread merely because a previous turn read the same resource: compaction may have removed it.
